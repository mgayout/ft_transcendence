...
class Tournament(models.Model):
    ...
    player_1 = models.ForeignKey(Player, on_delete=models.CASCADE, related_name='player1_tournament', null=True)
    player_2 = models.ForeignKey(Player, on_delete=models.CASCADE, related_name='player2_tournament', null=True, blank=True)
    player_3 = models.ForeignKey(Player, on_delete=models.CASCADE, related_name='player3_tournament', null=True, blank=True)
    player_4 = models.ForeignKey(Player, on_delete=models.CASCADE, related_name='player4_tournament', null=True, blank=True)
    status = models.CharField(choices=TournamentStatusChoices.choices, max_length=20, default=TournamentStatusChoices.OUVERT)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    winner = models.ForeignKey(Player, on_delete=models.SET_NULL, null=True, blank=True, related_name='won_tournaments')
    max_score_per_round = models.PositiveIntegerField(default=3)
    number_of_rounds    = models.PositiveIntegerField(default=3)

    def save(self, *args, **kwargs):
        # Si aucun nom n’est fourni et player_1 existe, définir un nom par défaut
...
class Match(models.Model):
    ...
    max_score_per_round = models.PositiveIntegerField(default=3)
    type = models.CharField(choices=TypeChoices.choices, max_length=10, default=TypeChoices.NORMAL)
