...
function FriendModal({ friend, setFriend }) {
	...
	const fonction = async (tab) => {
		...
		try {
			...
			if (tab != 'blocklist') {
				friendData = await axios.get('http://127.0.0.1:8000/api/friend/list/', {headers: config.headers, param: params})
				friendData = friendData.data
			}
			if (tab == 'addfriend' || tab == 'blocklist') {
				blockData = await axios.get('http://127.0.0.1:8000/api/block/list/', {headers: config.headers, param: params})
	...
	const removeFriend = async (playerID) => {
		try {
			const Atoken = localStorage.getItem('accessToken')
			const config = {headers: {Authorization: `Bearer ${Atoken}`}}
			const response = await axios.delete(`http://127.0.0.1:8000/api/friend/remove/${playerID}/`,
	...
	const addFriend = async (playerID) => {
		try {
			...
			const response = await axios.post('http://127.0.0.1:8000/api/friend-request/send/', params, config)
			fonction(tab)
		}
		catch(error) {
			console.log(error)
	...
	const addBlock = async (playerID) => {
		try {
			...
			const response = await axios.post('http://127.0.0.1:8000/api/block/add/', params, config)
			fonction(tab)
		}
		catch(error) {
			console.log(error)
	...
	const cancelRequest = async (playerID) => {
		try {
			const Atoken = localStorage.getItem('accessToken')
			const config = {headers: {Authorization: `Bearer ${Atoken}`}}
			const response = await axios.delete(`http://127.0.0.1:8000/api/friend-request/cancel/${playerID}/`,
				{headers: config.headers})
			fonction(tab)
		}
		catch(error) {
	...
	const acceptRequest = async (playerID) => {
		try {
			...
			const config = {headers: {Authorization: `Bearer ${Atoken}`}}
			const params = { player_2: playerID }
			const response = await axios.put(`http://127.0.0.1:8000/api/friend-request/accept/${playerID}/`, params, config)
			fonction(tab)
		}
		catch(error) {
			console.log(error)
	...
	const rejectRequest = async (playerID) => {
		try {
			const Atoken = localStorage.getItem('accessToken')
			const config = {headers: {Authorization: `Bearer ${Atoken}`}}
			const response = await axios.delete(`http://127.0.0.1:8000/api/friend-request/reject/${playerID}/`,
				{headers: config.headers})
			fonction(tab)
		}
		catch(error) {
	...
	const removeBlock = async (playerID) => {
		try {
			...
			const config = {headers: {Authorization: `Bearer ${Atoken}`}}
			//const params = { player_2: playerID }
			const response = await axios.delete(`http://127.0.0.1:8000/api/block/remove/${playerID}/`,
				{headers: config.headers})
			fonction(tab)
		}
		catch(error) {
